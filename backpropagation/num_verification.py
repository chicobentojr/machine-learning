...
@click.option('--alpha', '-a', default=bp.ALPHA, help='Weights Update Rate, is used to smooth the gradient')
@click.option('--beta', '-b', default=bp.BETA, help='Relevance of recent average direction (Method of Moment)')
...
def gradient_verification(network_filename, initial_weights_filename, data_set_filename,
    ...
    bp.logger = logger
    network = bp.single_backpropagation(network_filename, initial_weights_filename, data_set_filename, alpha, beta)
