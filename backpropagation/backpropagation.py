...
def backpropagation(network_filename, initial_weights_filename, data_set_filename, alpha):
    ...
    last_regularized_cost = regularized_cost(
        network, data_set, numExamples, True)
    max_iterations = 100
    stop = False
    iteration = 0
    ...
    while not stop and iteration < max_iterations:
        ...
        for k in range(0, numLayers-1):
            layer_k = network.layers[k]
            network.layers[k].weight_matrix.matrix -= (
                layer_k.gradient_matrix.matrix * alpha)
            logger.debug('\n\tTheta{} = \n{}'.format(
                k+1, network.layers[k].weight_matrix.str_tabs(2)))
