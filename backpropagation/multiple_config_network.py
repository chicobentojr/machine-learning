...
def cross_validation(dataset, k_fold, test_folder, original_labels, max_iterations, alpha,
    ...
    for f_index, (train_file, test_file) in enumerate(folds):
        ...
        network, training_result = bp.backpropagation(net_file, weights_file, train_file,
                                                      ...
                                                      validation_filename=test_file, possible_labels=original_labels,
...
def execute(filename, separator, k_fold, alpha, beta, regularizations, max_iterations,
    ...
    test_folder = 'inputs/tests/{}_{}'.format(now.strftime('%Y-%m-%d_%Hh%M'), data_file)
    ...
    test_results = cvn.get_empty_result_dict(original_labels)
